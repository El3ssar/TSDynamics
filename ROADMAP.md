...
| **2 — Rust backend** | 🟡 **~40%** | `backend="diffsol"` (pydiffsol, prebuilt LLVM wheels) **translates 100% of the 118 ODEs** and cross-validates against JiTCODE across the curated sample (pinned: `test_diffsol_translation.py` always-on + `test_diffsol_backend.py` slow; nightly installs the extra). ~11× on Lorenz. *Left:* promote diffsol to default ODE path (zero-compiler install); the bespoke `tsdynamics-core` PyO3/maturin crate for the **DDE** solver, **SDE** family, rayon ensembles; `benches/` in CI. |
