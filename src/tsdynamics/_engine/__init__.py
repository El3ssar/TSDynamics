...
.. note::
    **Advanced / internal.**  Reachable as :mod:`tsdynamics._engine` for inspection
...
- :mod:`~tsdynamics._engine.compile` — lowers a system's symbolic dynamics
...
- :mod:`~tsdynamics._engine.problem` — bundles a compiled tape with its runtime
  context (ic, parameters, delays, …) into a per-family ``Problem``.
- :mod:`~tsdynamics._engine.run` — backend selection (``interp`` / ``jit`` /
  ``reference``) and the ``integrate`` / ``ensemble`` entry points.
- :mod:`~tsdynamics._engine.symbols` — the engine-native ``y`` / ``t`` symbolic
...
# surface actually resolves (``tsdynamics._engine.run`` etc.) without the caller
