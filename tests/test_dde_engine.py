...
These exercise ``DelaySystem.run(backend="interp")``, which lowers a delay
...
def test_linear_dde_every_explicit_method(method):
    ...
    traj = _LinearDDE().run(
        backend="interp", solver=method, final_time=2.0, dt=dt, ic=[1.0], rtol=1e-9, atol=1e-11
    )
    i2 = int(np.argmin(np.abs(traj.t - 2.0)))
...
def test_every_builtin_dde_integrates_finite(name):
    ...
    te = sys.run(backend="interp", final_time=20.0, dt=0.25, history=history)
...
def test_engine_trajectory_provenance():
    te = _LinearDDE().run(backend="interp", solver="tsit5", final_time=1.0, dt=0.5, ic=[1.0])
    assert te.meta["engine"] == "rust"
...
def test_reference_backend_is_unsupported_for_dde():
    with pytest.raises(NotImplementedError):
        _LinearDDE().run(backend="reference", final_time=1.0, dt=0.1, ic=[1.0])
...
def test_implicit_method_is_rejected_for_dde():
    ...
    with pytest.raises(NotImplementedError, match="explicit"):
        _LinearDDE().run(backend="interp", solver="rosenbrock", final_time=1.0, dt=0.1, ic=[1.0])
...
def test_divergence_raises():
    ...
    with pytest.raises(RuntimeError):
        _BlowUp().run(backend="interp", final_time=100.0, dt=0.25, ic=[1.0])
...
def test_default_backend_is_the_engine():
    ...
    """
    traj = _LinearDDE().run(final_time=1.0, dt=0.5, ic=[1.0])
