...
def test_wrapped_step_and_state() -> None:
    w = ts.WrappedSystem(_logistic_step, dim=1, ic=[0.5])
    w.reinit()
    x1 = w.step()
...
def test_wrapped_trajectory_and_transient() -> None:
    w = ts.WrappedSystem(_logistic_step, dim=1, ic=[0.3])
...
def test_wrapped_named_components() -> None:
    ...
    w = ts.WrappedSystem(rot, dim=2, ic=[1.0, 0.0], variables=("x", "y"))
    traj = w.run(20)
...
def test_wrapped_trajectory_final_time_dt_continuous() -> None:
    ...
    w = ts.WrappedSystem(_expanding_flow, dim=1, family="ode", default_dt=0.1)
...
def test_wrapped_trajectory_final_time_uses_default_dt() -> None:
    ...
    w = ts.WrappedSystem(_expanding_flow, dim=1, family="ode", default_dt=0.25)
