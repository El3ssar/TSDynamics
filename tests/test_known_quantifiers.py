...
def test_rqa_determinism_of_a_periodic_signal_is_one():
    ...
    """
    emb = ts.embed(sinusoid(2048, freq=0.02), dimension=_EMB_M, delay=_EMB_TAU)
    det = ts.rqa(emb, recurrence_rate=_RR, theiler=_THEILER).determinism
...
def test_rqa_determinism_of_white_noise_is_near_zero():
    ...
    emb = ts.embed(white_noise(2048, seed=11), dimension=_EMB_M, delay=_EMB_TAU)
    det = ts.rqa(emb, recurrence_rate=_RR, theiler=_THEILER).determinism
...
def test_lyapunov_from_data_logistic_r4_is_ln2():
    ...
    """
    lam = float(ts.lyapunov_from_data(logistic_series(4000, r=4.0), dimension=_EMB_M, delay=1))
...
def test_lyapunov_from_data_periodic_is_near_zero():
    ...
    lam = float(ts.lyapunov_from_data(_periodic_signal(), dimension=_EMB_M, delay=_EMB_TAU))
...
def _det(signal: np.ndarray) -> float:
    ...
    return float(ts.rqa(emb, recurrence_rate=_RR, theiler=_THEILER).determinism)
...
def _complexity_quantifiers(signal: np.ndarray) -> dict[str, float]:
    ...
    embedded = ts.embed(signal, dimension=_EMB_M, delay=_EMB_TAU)
    return {
        "correlation_dimension": float(ts.correlation_dimension(embedded, theiler=_THEILER)),
        "zero_one_test": float(ts.zero_one_test(signal, seed=0)),
        "lyapunov_from_data": float(
            ts.lyapunov_from_data(signal, dimension=_EMB_M, delay=_EMB_TAU)
        ),
    }
...
def test_system_quantifiers_agree_chaotic_above_regular():
    ...
    lam_c = float(ts.max_lyapunov(chaotic, n=3000, ic=_CHAOTIC_IC))
    lam_r = float(ts.max_lyapunov(regular, n=3000, ic=_REGULAR_IC))
    ...
    gali_c = float(ts.gali(chaotic, k=2, n=500, ic=_CHAOTIC_IC))
    gali_r = float(ts.gali(regular, k=2, n=500, ic=_REGULAR_IC))
    assert gali_c < 1.0e-8
    assert gali_c < gali_r

    # The 0--1 test on the first component: K ≈ 1 (chaotic) vs K ≈ 0 (regular).
    k_c = float(ts.zero_one_test(chaotic, n=3000, ic=_CHAOTIC_IC, component=0, seed=0))
    k_r = float(ts.zero_one_test(regular, n=3000, ic=_REGULAR_IC, component=0, seed=0))
    assert k_c - k_r > 0.5

    # A fractal attractor (D2 ≈ 1.22 for Hénon) vs a smooth invariant curve (≈ 1).
    d2_c = float(ts.correlation_dimension(chaotic.iterate(steps=3000, ic=_CHAOTIC_IC)))
    d2_r = float(ts.correlation_dimension(regular.iterate(steps=3000, ic=_REGULAR_IC)))
    assert d2_c > d2_r
