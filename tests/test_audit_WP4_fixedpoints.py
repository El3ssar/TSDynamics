...
def test_interval_encloses_exp_of_state() -> None:
    ...
    region = ts.data.Box([-3.0] * sys.dim, [3.0] * sys.dim)
    fps = ts.analysis.fixed_points(sys, method="interval", region=region).details
    assert len(fps) >= 1
    for p in fps:
...
def test_interval_polynomial_flow_unaffected() -> None:
    ...
    region = ts.data.Box([-30.0] * 3, [30.0] * 3)
    pts = sorted(
        tuple(round(c, 5) for c in p.x)
        for p in list(ts.analysis.fixed_points(sys, method="interval", region=region).details)
    )
