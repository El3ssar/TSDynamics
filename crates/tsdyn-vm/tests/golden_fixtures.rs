//! The production interpreter, run over real v2-emitted tapes.
//!
//! Stream F1 froze a set of golden fixtures under `tsdyn-ir/tests/fixtures/`:
//! each holds the integer tape arrays the v2 Python emitter produced for a real
//! system, plus the right-hand side and Jacobian sampled at fixed checkpoints by
//! the system's own symbolic engine (the engine-independent ground truth).
//!
//! This test rebuilds each tape, evaluates it with [`Interpreter`], and asserts
//! it (a) reproduces the symbolic checkpoints to ~1e-12 — i.e. matches the v2
//! tape's intended right-hand side on real systems — and (b) agrees with the
//! `reference` evaluator **bit-for-bit**. The fixtures are read from the F1 crate
//! (a frozen, read-only contract artifact, exactly the cross-evaluator oracle
//! its module docs designate); this stream does not own or edit them.
//!
...
fn assert_close(label: &str, got: &[f64], want: &[f64]) {
    ...
    for (i, (g, w)) in got.iter().zip(want.iter()).enumerate() {
        ...
        assert!(
            (g - w).abs() <= tol,
            "{label}[{i}]: interpreter gave {g}, symbolic ground truth {w} (|Δ| = {:e})",
            (g - w).abs()
        );
    }
}
...
fn check_fixture(path: &Path) {
    ...
    for (c, cp) in fx.checkpoints.iter().enumerate() {
        let (deriv, jac) = interp.eval_jac_alloc(&cp.u, &fx.params, cp.t);
...
#[test]
fn interpreter_reproduces_all_golden_fixtures() {
