...
/// Adaptive L-stable stiff trajectory (linearly-implicit Euler + Richardson)
/// with Hermite dense output at every `t_eval` point. `jac_outputs` carries the
/// analytic Jacobian register indices (row-major `dim × dim`).
#[pyfunction]
#[allow(clippy::too_many_arguments)]
fn integrate_dense_stiff_py<'py>(
    py: Python<'py>,
    ops: PyReadonlyArray1<i32>,
    a: PyReadonlyArray1<i32>,
    b: PyReadonlyArray1<i32>,
    ...
    let mut tape = make_tape(ops, a, b, imm, outputs, n_state, n_param);
    tape.jac_outputs = jac_outputs.as_slice().unwrap().to_vec();
    let dim = tape.dim();
    let u0 = u0.as_slice().unwrap().to_vec();
    let p = p.as_slice().unwrap().to_vec();
    let t_eval = t_eval.as_slice().unwrap().to_vec();
    let flat = py.detach(|| rosenbrock::integrate_dense(&tape, &u0, &p, &t_eval, rtol, atol));
    let n_t = t_eval.len();
    PyArray1::from_vec(py, flat).reshape([n_t, dim]).unwrap()
}

/// Adaptive stiff ensemble: integrate a batch of ICs from `t0` to `t1` in
/// parallel (rayon), returning each final state as `(n_ic, dim)`.
#[pyfunction]
#[allow(clippy::too_many_arguments)]
fn integrate_ensemble_final_stiff_py<'py>(
    py: Python<'py>,
    ops: PyReadonlyArray1<i32>,
    a: PyReadonlyArray1<i32>,
    b: PyReadonlyArray1<i32>,
    ...
    let flat = py.detach(|| {
        let rows: Vec<Vec<f64>> = (0..n_ic)
            .into_par_iter()
            .map_init(
                || rosenbrock::StiffWorkspace::for_tape(&tape),
                |ws, i| {
                    let u0: Vec<f64> = batch.row(i).to_vec();
                    rosenbrock::integrate_final(&tape, &u0, &p, t0, t1, rtol, atol, ws)
                },
            )
            .collect();
        let mut out = Vec::with_capacity(n_ic * dim);
...
fn tsdynamics_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(integrate_dense_stiff_py, m)?)?;
    m.add_function(wrap_pyfunction!(integrate_ensemble_final_stiff_py, m)?)?;
    Ok(())
}
